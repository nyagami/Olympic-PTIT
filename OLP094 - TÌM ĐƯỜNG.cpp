...
void dfs(ll x, ll y, ll dist)
{
    if (vis[x][y] || dist > 2)
        return;
    vis[x][y] == 1;
    if (finish.fi == x && finish.se == y)
    {
        flag = 1;
        return;
    }
    for (ll i = x, j = y; i <= n && !vis[i][j]; i++)
    {
        dfs(i, j, dist + 1);
    }
    for (ll i = x, j = y; i && !vis[i][j]; i--)
    {
        dfs(i, j, dist + 1);
    }
    for (ll i = x, j = y; j <= m && !vis[i][j]; j++)
    {
        dfs(i, j, dist + 1);
    }
    for (ll i = x, j = y; j && !vis[i][j]; j--)
    {
        dfs(i, j, dist + 1);
    }
}
